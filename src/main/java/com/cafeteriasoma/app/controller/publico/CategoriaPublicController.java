...
public class CategoriaPublicController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<CategoriaResponse> obtenerPorId(@PathVariable("id") @Positive Long idCategoria) {
        return ResponseEntity.ok(categoriaService.obtenerActivaPorId(idCategoria));
    }
}
